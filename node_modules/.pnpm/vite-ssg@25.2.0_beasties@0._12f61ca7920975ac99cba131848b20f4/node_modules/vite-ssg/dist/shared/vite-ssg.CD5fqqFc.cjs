...
function buildLog(text, count) {
  console.log(`
${ansis.gray("[vite-ssg]")} ${ansis.yellow(text)}${count ? ansis.blue(` (${count})`) : ""}`);
}
...
async function build(ssgOptions = {}, viteConfig = {}) {
  ...
  if (node_fs.existsSync(ssgOutTempFolder))
    await fs__default.rm(ssgOutTempFolder, { recursive: true });
  ...
  const _require = node_module.createRequire((typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === 'SCRIPT' && _documentCurrentScript.src || new URL('shared/vite-ssg.CD5fqqFc.cjs', document.baseURI).href)));
  ...
  if (beasties)
    console.log(`${ansis.gray("[vite-ssg]")} ${ansis.blue("Critical CSS generation enabled via `beasties`")}`);
  const {
  ...
  for (const route of routesPaths) {
    queue.add(async () => {
      try {
        ...
        await fs__default.mkdir(node_path.join(out, node_path.dirname(filename)), { recursive: true });
        await fs__default.writeFile(node_path.join(out, filename), formatted, "utf-8");
        config.logger.info(
          `${ansis.dim(`${outDir}/`)}${ansis.cyan(filename.padEnd(15, " "))}  ${ansis.dim(getSize(formatted))}`
        );
      } catch (err) {
        throw new Error(`${ansis.gray("[vite-ssg]")} ${ansis.red(`Error on page: ${ansis.cyan(route)}`)}
${err.stack}`);
      }
    });
  }
  await queue.start().onIdle();
  await fs__default.rm(ssgOutTempFolder, { recursive: true, force: true });
  ...
  console.log(`
${ansis.gray("[vite-ssg]")} ${ansis.green("Build finished.")}`);
  await onFinished?.();
  const waitInSeconds = 15;
  const timeout = setTimeout(() => {
    console.log(`${ansis.gray("[vite-ssg]")} ${ansis.yellow(`Build process still running after ${waitInSeconds}s. There might be something misconfigured in your setup. Force exit.`)}`);
